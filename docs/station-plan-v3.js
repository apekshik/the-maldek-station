...
(() => {
  ...
  const zones = {
    ...
    secondary: ['Second deck descent / proposed', 'Run the second stair along the outer right/east edge of the bypass, parallel and attached to the deck. Ascend north, then turn left across a flush landing into the bypass. The lower landing connects to the approach/service path. This is a proposed second public-deck access, not a claim that this precise stair already exists. Keep the existing eastern machinery/service stair distinct. Verify landing level and terrain before fixing the flight.'],
  ...
  panel.innerHTML = `<div class="plan-container">
    ...
    <rect x="440" y="450" width="20" height="40" fill="#324138" stroke="#96b89c"/>
    ${room('secondary','<rect x="460" y="450" width="40" height="200" fill="#293c32" stroke="#96b89c" stroke-width="2"/>',525,481,'SIDE-MOUNTED STAIR / PROPOSED')}
    ${Array.from({length:13},(_,i)=>`<path d="M464 ${495+i*8}H496" stroke="#88aa91"/>`).join('')}
    <path d="M420 470H480V810H145" fill="none" stroke="#96b89c" stroke-width="2" stroke-dasharray="6 4"/>
