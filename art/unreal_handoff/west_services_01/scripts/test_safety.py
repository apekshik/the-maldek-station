...
OUT=Path(__file__).resolve().parents[1];dest=OUT;ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);assert not ls.is_in_play_in_editor()
...
o=json.loads((OUT/'baseline.json').read_text())['origin'];s={'phase':0,'index':0,'next':time.monotonic()+12,'deadline':time.monotonic()+180,'checks':{},'stops':{}}
...
def tick(dt):
 ...
 try:
  ...
  phase=s['phase']
  if phase==0:
   actors=sorted([a for a in unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationCabinet) if a.get_actor_label().startswith('MIG_WS_')],key=lambda a:a.get_actor_label());check('seventeen_mechanisms',len(actors)==17)
  ...
  if phase==1:
   ...
   if 'Secure_tray' in label:
    door=next(a for a in actors if a.get_actor_label()=='MIG_WS_WSP_Secure_door_hinge')
    if door.get_open_fraction()<.99:door.try_interact();advance(1,1.5);return
   ...
   q=unreal.MathLibrary.transform_location(d.get_actor_transform(),unreal.MathLibrary.transform_location(half,f));p.character_movement.stop_movement_immediately();p.set_actor_location(q,False,True);d.try_interact();advance(2,1.3);return
  if phase==2:
