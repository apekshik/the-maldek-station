...
OUT=Path(__file__).resolve().parents[1];REPO=OUT.parents[2];dest=OUT/'east_wall'
for sub in ['fbx','textures']:(dest/sub).mkdir(parents=True,exist_ok=True)
src=REPO/'art/blender/passenger_lodge_wall_details_02/Maldek_East_Wall_Details.blend';sha=hashlib.sha256(src.read_bytes()).hexdigest()
bpy.ops.wm.open_mainfile(filepath=str(src));bpy.context.scene.frame_set(1);dg=bpy.context.evaluated_depsgraph_get();kit=bpy.data.collections['PLG2_East_Wall_Details']
...
def printed(o):return any(m.name.startswith('PLG2_Ink_') for m in o.data.materials)
...
for id in ['Frames']:
 ...
 pivot=bpy.data.objects['PLG2_Cabinet_LeafPivot'];latch=bpy.data.objects['PLG2_Cabinet_turn_latch']
 H=Matrix.Translation(pivot.matrix_world.translation);frames={'Static':H,'Leaf':H,'Latch':Matrix.Translation(latch.matrix_world.translation)};controls={'Leaf':{'open_degrees':-95,'source_axis':'world Z'}};keys=list(frames);copies=[];sources={k:[] for k in keys};collision={k:[] for k in keys}
 for ob in objs:
  role='Latch' if ob==latch else 'Leaf' if parented(ob,pivot) else 'Static'
  if ob.name=='PLG2_Cabinet_leaf':
   ...
   collision['Leaf'].append({'source':ob.name,'center':[(a+b)/2 for a,b in zip(low,high)],'extent':[(b-a)/2 for a,b in zip(low,high)]})
 ...
 for channel in ['BaseColor','ORM','NormalGL']:
  im=bpy.data.images.new('PLG2_'+id+'_'+channel,4096,4096,alpha=False);im.colorspace_settings.name='sRGB' if channel=='BaseColor' else 'Non-Color';im.generated_color=(.5,.5,1,1) if channel=='NormalGL' else (0,0,0,1)
...
for ob in objects:
 ...
 for m in me.materials:
  if m.name.startswith('PLG2_Ink_') and m.name not in printed_materials:
 ...
 groups[0]['parts'].append({'role':ob.name,'mesh':name,'matrix':[list(r) for r in H],'local_matrix':[list(r) for r in Matrix.Identity(4)],'control':{'parent':'Leaf'} if parented(ob,pivot) else None,'collision':[],'sources':[ob.name]});bpy.data.objects.remove(part,do_unlink=True)
...
(dest/'exports.json').write_text(json.dumps({'source_sha256':sha,'source_objects':inventory,'groups':groups,'assets':assets,'printed_materials':printed_materials,'bake':'4K frame/metal/wood atlas; original printed artwork and source UVs retained; independent first-aid hinge and latch; separate instrument glass.'},indent=2));print('WALL_EXPORT_COMPLETE',flush=True)
