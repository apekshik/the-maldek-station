"""Fit three native StationDoor actors; retain all independent authored controls."""
import unreal,json,hashlib,re
from pathlib import Path
OUT=Path(__file__).resolve().parents[1];REPO=OUT.parents[2];dest=OUT/'doors';root='/Game/MaldekRefinement/PassengerLodge/Doors'
...
for row in data['doors']:
 ...
 for channel,trow in row['textures'].items():
  file=dest/trow['file'];assert hashlib.sha256(file.read_bytes()).hexdigest()==trow['sha256'];name='T_PLD_'+id+'_'+channel;path=root+'/Textures/'+name;tex=lib.load_asset(path)
  if not tex:
 ...
 name='M_PLD_'+id;path=root+'/Materials/'+name;m=lib.load_asset(path) or at.create_asset(name,root+'/Materials',unreal.Material,unreal.MaterialFactoryNew());ml.delete_all_material_expressions(m)
...
glass=lib.load_asset('/Game/MaldekRefinement/PassengerLodge/Surfaces/Materials/M_PLW_Opal_Glass_6mm');assert glass
...
for row in data['assets']:
 ...
 assert len(mesh.static_materials)==1;mesh.set_material(0,glass if row['role']=='Glass' else materials[row['door']]);sm.remove_collisions(mesh)
 if row['role']=='FixedHardware':mesh.get_editor_property('body_setup').set_editor_property('collision_trace_flag',unreal.CollisionTraceFlag.CTF_USE_COMPLEX_AS_SIMPLE)
...
standard=actors['R12_Door_Control_side'];assert standard.service_key.static_mesh
components={'Leaf':'leaf','Glass':'glass','FixedHardware':'fixed_hardware','KeyHousing':'key_housing','KeyPlug':'key_plug','InteriorKeyPlug':'interior_key_plug','FrontLever':'front_lever','BackLever':'back_lever','MovingLatch':'moving_latch','BottomSeal':'bottom_seal'}
def vec(p):return unreal.Vector(100*p[0],-100*p[1],100*p[2])
placed=[]
for row in data['doors']:
 id=row['id'];label='MIG_PLD_'+id;H=row['hinge_matrix'];p=[H[j][3] for j in range(3)];loc=unreal.Vector(o[0]-100*p[0],o[1]+100*p[1],o[2]+100*p[2]);rot=unreal.Rotator(pitch=0,yaw=180-row['yaw'],roll=0)
 a=actors.get(label) or aa.spawn_actor_from_class(unreal.StationDoor,loc,rot);a.set_actor_label(label);a.set_folder_path('LodgeMigration/Doors')
 for prop,value in {'use_authored_hardware':True,'has_keypad':False,'has_key_lock':True,'key_available':True,'locked':True,'relock_on_close':True,'key_face_depth':3.4,'interior_is_negative_y':row['exterior']<0,'open_angle':row['open_angle'],'degrees_per_second':75.0,'authored_key_location':vec(row['key_location_source']),'authored_key_rotation':unreal.Rotator(pitch=0,yaw=row['key_yaw'],roll=0)}.items():a.set_editor_property(prop,value)
 a.key_lock_root.set_relative_location(vec(row['key_location_source']),False,False);a.key_lock_root.set_relative_rotation(unreal.Rotator(pitch=0,yaw=row['key_yaw'],roll=0),False,False)
 for part in row['parts']:
  ...
  if part['role'] in ['FrontLever','BackLever']:c.set_relative_location(vec([part['local_matrix'][j][3] for j in range(3)]),False,False)
 a.fixed_hardware.set_collision_profile_name('BlockAll');a.key_plug.set_relative_location(unreal.Vector(0,3.4,0),False,False);a.interior_key_plug.set_relative_location(unreal.Vector(0,-3.4,0),False,False)
 a.service_key.set_static_mesh(standard.service_key.static_mesh);a.service_key.set_relative_location(unreal.Vector(0,3.4,0),False,False);a.service_key.set_visibility(False)
 box=row['leaf_collision_source'];a.leaf_collision.set_relative_location(vec(box['center']),False,False);a.leaf_collision.set_box_extent(unreal.Vector(*[v*100 for v in box['extent']]),False)
 for prop in ['unlatch_sound','movement_sound','closing_movement_sound','close_sound','unlock_sound','lock_sound','key_turn_sound','key_turn_volume','door_volume','movement_volume']:
  a.set_editor_property(prop,standard.get_editor_property(prop))
 for audio in a.get_components_by_class(unreal.AudioComponent):audio.set_relative_location(vec([row['width']/2,0,1.1]),False,False)
 a.get_editor_property('KeypadDisplay').set_visibility(False);a.key_inspection_light.set_visibility(False);placed.append({'label':label,'position':p,'yaw':180-row['yaw'],'open_angle':row['open_angle'],'key_available':True})
