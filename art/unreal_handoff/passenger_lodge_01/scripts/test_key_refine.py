...
OUT=Path(__file__).resolve().parents[1];out=OUT/'doors';shots=OUT/'previews/key_refine';shots.mkdir(exist_ok=True);ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem)
...
def finish(error=None):
 unreal.StationMigrationLibrary.send_pie_key('LeftMouseButton',False);unreal.StationMigrationLibrary.send_pie_key('E',False);settings.set_editor_property('bThrottleCPUWhenNotForeground',throttle);unreal.StationMigrationLibrary.set_pie_render_size(0,0);s.update(success=error is None,error=error);(OUT/'audio_refine/key_runtime.json').write_text(json.dumps({k:v for k,v in s.items() if k!='busy'},indent=2));unreal.unregister_slate_post_tick_callback(handle);ls.editor_request_end_play()
def tick(dt):
 ...
 try:
  ...
  if phase==4:
   check('E_enters_key_camera',d.is_using_key() and pc.get_view_target()==d);check('input_held',pc.is_move_input_ignored() and pc.is_look_input_ignored());cam=d.get_editor_property('KeypadCamera').get_world_location()
   rays=[]
   for fraction in [0,.5,1]:
    target=d.get_key_grip_world_position(fraction);hit=unreal.SystemLibrary.line_trace_single(w,cam,target,unreal.TraceTypeQuery.TRACE_TYPE_QUERY1,True,[p],unreal.DrawDebugTrace.NONE,True);h=hit.to_tuple() if hit else None;rays.append({'fraction':fraction,'blocked':bool(h and h[0]),'actor':h[9].get_actor_label() if h and h[0] and h[9] else None})
   s.setdefault('camera_rays',{})[d.get_actor_label()]=rays
   check('key_path_visible',not any(r['blocked'] for r in rays));shot('key_ready');advance(4.5,1);return
