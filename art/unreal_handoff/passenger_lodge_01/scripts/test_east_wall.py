...
OUT=Path(__file__).resolve().parents[1];dest=OUT/'east_wall';shots=OUT/'previews/east_wall_night';shots.mkdir(exist_ok=True);ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);assert not ls.is_in_play_in_editor()
...
o=json.loads((OUT/'before.json').read_text())['origin'];data=json.loads((OUT/'east_wall/exports.json').read_text());specs=[('MIG_PLG2_FirstAid',next(p for p in data['groups'][0]['parts'] if p['role']=='Leaf'))]
...
def put():
 ...
 point=unreal.Vector(o[0]+1175,o[1]-510,o[2]+499);p.set_actor_location(point,False,True)
def aim():
...
def tick(dt):
 ...
 try:
  ...
  if phase==2:
   ...
   check(label+'_focus',d.interaction_prompt.is_visible());shot(label+'_closed');key(True);advance(3,.15);return
  ...
  if phase==4:
   ...
   check(label+'_fully_open',abs(d.get_open_fraction()-1)<.001);check(label+'_not_obstructed',not d.is_obstructed());check(label+'_recorded_audio',len(d.opening_takes)==2 and len(d.closing_takes)==2);aim();shot(label+'_open');advance(5,1.2);return
  if phase==5:check(label+'_open_focus',d.interaction_prompt.is_visible());key(True);advance(6,.15);return
  ...
  if phase==40:
   s['actors']={};s['route_data']=[(k,v) for k,v in json.loads((dest/'routes.json').read_text()).items() if k.startswith('East') or k=='First_aid_approach'];s['route_index']=0
   ...
   for a in unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationCabinet):
    if a.get_actor_label().startswith('MIG_PLL_') or a.get_actor_label()=='MIG_PLG2_FirstAid':a.try_interact()
