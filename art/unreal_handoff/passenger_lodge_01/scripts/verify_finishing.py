...
OUT=Path(__file__).resolve().parents[1];REPO=OUT.parents[2];dest=OUT/'finishing';ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);aa=unreal.get_editor_subsystem(unreal.EditorActorSubsystem)
...
w=unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem).get_editor_world();by={a.get_actor_label():a for a in aa.get_all_level_actors()};base=json.loads((OUT/'before.json').read_text());o=base['origin'];previous=json.loads((OUT/'lockers/before.json').read_text())
...
assert len(previous)==1200 and len(by)==1247
errors=[];materials=[];textures=[];boxes=0
for package,folder,prefix in [('lockers','Lockers','PLL'),('wall_displays','WallDisplays','PLG')]:
...
assert not any(a.get_actor_label().startswith('Lodge_Surface_Review_Temporary') for a in by.values())
