...
def finish(error=None):
 unreal.StationMigrationLibrary.send_pie_key('E',False);s.update(success=error is None,error=error);(OUT/'doors/safety.json').write_text(json.dumps(s,indent=2));unreal.unregister_slate_post_tick_callback(handle);ls.editor_request_end_play()
def tick(dt):
 ...
 try:
  ...
  if phase==0:
   ...
   advance(1);return
  if phase==1:
   if s['index']==3:advance(10);return
   label=['MIG_PLD_ARRIVAL','MIG_PLD_GONDOLA','MIG_PLD_STAFF'][s['index']];d=doors[label];put(d,70,70*(1 if d.open_angle>0 else -1));d.unlock();d.try_interact();advance(2,2);return
  if phase==2:
   angle=abs(d.get_open_angle());check(label+'_blocked_before_full_swing',.1<angle<80);s.setdefault('blocked_angles',{})[label]=angle;put(d,60,160);d.try_interact();advance(3,2);return
  if phase==3:check(label+'_safe_close',abs(d.get_open_angle())<.01 and d.is_locked());d.unlock();d.try_interact();advance(4,2);return
  if phase==4:check(label+'_clear_reopen',abs(d.get_open_angle()-d.open_angle)<.1);d.try_interact();advance(5,2);return
  if phase==5:check(label+'_final_closed',d.is_locked());s['index']+=1;advance(1);return
  if phase==10:
