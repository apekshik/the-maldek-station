"""Bake weathered PLK finishes and retain fifteen hinge/slide controls.
The integrated Blender master is read only; existing hatch/walls are retained.
"""
import bpy,json,hashlib,math,bmesh,sys
from pathlib import Path
from mathutils import Vector,Matrix
OUT=Path(__file__).resolve().parents[1];REPO=OUT.parents[2];dest=OUT/'kitchen'
...
bpy.ops.wm.open_mainfile(filepath=str(src));bpy.context.scene.frame_set(1);dg=bpy.context.evaluated_depsgraph_get();kit=bpy.data.collections['PLK_Assets']
...
main=['Serving','Hot_base','Prep_drawers','Wash_base','Rear_left','Rear_right','Fridge','Microwave']
def family(ob):
 q=ob
 while q.parent and q.parent.name!='PLK_Assembly':q=q.parent
 return q.name.removeprefix('PLK_')
prior=json.loads((dest/'exports.json').read_text()) if '--fixtures-only' in sys.argv else None
for id in main+['Fixtures']:
 if prior and id!='Fixtures':
  groups.extend(g for g in prior['groups'] if g['id']==id);assets.extend(a for a in prior['assets'] if a['group']==id);continue
 objs=[ob for ob in objects if ob.type in {'MESH','FONT','CURVE'} and (family(ob)==id if id!='Fixtures' else family(ob) not in main)]
 pivots=[p for p in objects if p.get('motion') in {'hinge','slide'} and family(p)==id]
 H=Matrix.Translation(Vector((-24.1,4,4)))
 frames={'Static':H};controls={}
 if id=='Fixtures':frames['Glass']=H
 for p in pivots:frames[p.name]=p.matrix_world.copy();controls[p.name]=dict(p.items())
 keys=list(frames);copies=[];sources={k:[] for k in keys};collision={k:[] for k in keys}
 for ob in objs:
  role='Glass' if any('Display_glass' in m.name for m in ob.data.materials) else next((p.name for p in pivots if parented(ob,p)),'Static')
  # Each source piece supplies a tight box; keep drawer cavities empty.
  if role in controls and ob.type=='MESH' and 'hinge' not in ob.name.lower():
   pts=[frames[role].inverted()@ob.matrix_world@Vector(v) for v in ob.bound_box];low=[min(v[j] for v in pts) for j in range(3)];high=[max(v[j] for v in pts) for j in range(3)]
 ...
 for channel in ['BaseColor','ORM','NormalGL']:
  im=bpy.data.images.new('PLK_'+id+'_'+channel,4096 if id=='Fixtures' else 2048,4096 if id=='Fixtures' else 2048,alpha=False);im.colorspace_settings.name='sRGB' if channel=='BaseColor' else 'Non-Color';im.generated_color=(.5,.5,1,1) if channel=='NormalGL' else (0,0,0,1)
...
(dest/'exports.json').write_text(json.dumps({'source_sha256':sha,'source_objects':inventory,'groups':groups,'assets':assets,'bake':'2K storage/appliance atlases and 4K fixtures; source coordinates retained. Fifteen independent hinge/slide controls; kettle and urn lids remain closed.'},indent=2));print('KITCHEN_EXPORT_COMPLETE',flush=True)
