...
def finish(error=None):
 ...
 s.update(success=error is None,error=error);(out/'peek_dark_runtime.json').write_text(json.dumps(s,indent=2));ls.editor_request_end_play()
...
def tick(dt):
 ...
 try:
  ...
  if n==0:
   ...
   check('torch_stays_on_player',beam.get_attach_parent()==parent)
   mouse=pc.get_mouse_position() or pointer;key('Q',True);step(3)
  elif n==3:
   check('left_from_face_95',abs(delta(cam.get_editor_property('relative_rotation').yaw,facing)+95)<.2)
   ...
   key('LeftMouseButton',True);step(4,.7)
  elif n==4:
   r=beam.get_editor_property('relative_rotation');s['aim']={'yaw':r.yaw,'pitch':r.pitch}
   check('mouse_cannot_aim_torch',abs(delta(r.yaw,beamrest[1]))<.1 and abs(delta(r.pitch,beamrest[0]))<.1);check('torch_still_on_player',beam.get_attach_parent()==parent)
   ...
   check('light_power_preserved',abs(beam.intensity-s['beam_intensity'])<.01 and abs(beam.outer_cone_angle-s['beam_cone'])<.01)
   ...
   r=beam.get_editor_property('relative_rotation');check('torch_unchanged_on_return',abs(delta(r.yaw,beamrest[1]))<.1 and abs(delta(r.pitch,beamrest[0]))<.1)
   pointer=mouse
   key('E',True);step(6)
  elif n==6:
   ...
   check('torch_parent_restored',beam.get_attach_parent()==parent)
