"""Bake PLR fixture finishes and retain door, lever, bolt and indicator controls.
The integrated source is read only; opening patches are already in the shell.
"""
import bpy,json,hashlib,math,bmesh
from pathlib import Path
from mathutils import Vector,Matrix
OUT=Path(__file__).resolve().parents[1];REPO=OUT.parents[2];dest=OUT/'restrooms'
...
bpy.ops.wm.open_mainfile(filepath=str(src));bpy.context.scene.frame_set(1);dg=bpy.context.evaluated_depsgraph_get();kit=bpy.data.collections['PLR_Assets']
...
for id in ['Women_Entry','Men_Entry','Women_A','Women_B','Men_A','Women_Basin','Men_Basin','Men_Urinal']:
 prefix='PLR_'+id
 def belongs(ob):
  if id=='Women_Basin':return ob.name.startswith('PLR_Women_') and not ob.name.startswith(('PLR_Women_Entry','PLR_Women_A_','PLR_Women_B_'))
  if id=='Men_Basin':return ob.name.startswith('PLR_Men_') and not ob.name.startswith(('PLR_Men_Entry','PLR_Men_A_','PLR_Men_Urinal'))
  return ob.name.startswith(prefix+'_')
 objs=[ob for ob in objects if belongs(ob) and ob.type in {'MESH','FONT','CURVE'}]
 pivot=bpy.data.objects.get(prefix+('_Pivot' if id.endswith('Entry') else '_Stall_Pivot'))
 H=pivot.matrix_world.copy() if pivot else Matrix.Translation(Vector((-24.1,4,4)))
 frames=({'FixedHardware':H} if id.endswith('Entry') else {'Static':H});leaf=None
 if pivot:
  frames['FixedHardware']=H
  leaf=next(ob for ob in objs if ob.name.endswith('_Leaf'));frames['Leaf']=H
  for face,role in [('-1','FrontLever'),('1','BackLever')]:
   lever=next(ob for ob in objs if ob.name.endswith('_Lever_'+face));v=lever.matrix_world.translation.copy()
   # The lever root is at the latch rose, not the centre of its curved grip.
   rose=next(ob for ob in objs if ob.name.endswith('_Latch_rose_'+face));v.x=rose.matrix_world.translation.x
   frames[role]=Matrix.Translation(v)
  if not id.endswith('Entry'):
   frames['MovingLatch']=H;frames['Indicator']=H
 keys=list(frames);copies=[];sources={k:[] for k in keys}
 for ob in objs:
  role='FixedHardware' if id.endswith('Entry') or '_Stall_Hinge_fixed' in ob.name or '_Stall_Hinge_barrel' in ob.name or ob.name.endswith('_Stall_Stop') else 'Static'
  if pivot and parented(ob,pivot):
   role='Leaf'
   if ob.name.endswith('_Lever_-1'):role='FrontLever'
   elif ob.name.endswith('_Lever_1'):role='BackLever'
   elif ob.name.endswith('_Slide_bolt'):role='MovingLatch'
   elif ob.name.endswith('_Indicator_VACANT'):role='Indicator'
 ...
 for channel in ['BaseColor','ORM','NormalGL']:
  im=bpy.data.images.new('PLR_'+id+'_'+channel,2048,2048,alpha=False);im.colorspace_settings.name='sRGB' if channel=='BaseColor' else 'Non-Color';im.generated_color=(.5,.5,1,1) if channel=='NormalGL' else (0,0,0,1)
 ...
 for role,target in frames.items():
  ...
  parts.append({'role':role,'mesh':name,'local_matrix':[list(r) for r in H.inverted()@target],'sources':sources[role]});bpy.data.objects.remove(part,do_unlink=True)
 row={'id':id,'matrix':[list(r) for r in H],'textures':textures,'parts':parts,'door':bool(pivot)}
 if pivot:
  pts=[H.inverted()@leaf.matrix_world@Vector(v) for v in leaf.bound_box];lo=[min(v[j] for v in pts) for j in range(3)];hi=[max(v[j] for v in pts) for j in range(3)]
  row.update(open_angle=-float(pivot['open_angle_degrees']),width=float(pivot['leaf_width_m']),entry=id.endswith('Entry'),leaf_collision_source={'center':[(a+b)/2 for a,b in zip(lo,hi)],'extent':[(b-a)/2 for a,b in zip(lo,hi)]})
  if not row['entry']:
   bolt=next(q for q in objs if q.name.endswith('_Slide_bolt'));row['bolt_focus_source']=list(H.inverted()@bolt.matrix_world.translation)
...
(dest/'exports.json').write_text(json.dumps({'source_sha256':sha,'source_objects':inventory,'groups':groups,'assets':assets,'bake':'2K BaseColor/ORM/NormalGL per assembly; source metric UV and PLR_Deposit attribute retained. Independent doors, levers, stall bolts and indicators.'},indent=2));print('RESTROOM_EXPORT_COMPLETE',flush=True)
