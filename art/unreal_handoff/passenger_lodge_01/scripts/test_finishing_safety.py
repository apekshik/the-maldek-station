"""Every locker and kitchen mechanism stops for a player obstruction and retries safely."""
import unreal,json,time,traceback
from pathlib import Path
OUT=Path(__file__).resolve().parents[1];dest=OUT/'finishing';ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);assert not ls.is_in_play_in_editor()
...
o=json.loads((OUT/'before.json').read_text())['origin'];s={'phase':0,'index':0,'next':time.monotonic()+12,'deadline':time.monotonic()+300,'checks':{},'stops':{}}
...
def tick(dt):
 ...
 try:
  ...
  phase=s['phase']
  if phase==0:
   actors=sorted(unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationCabinet),key=lambda a:a.get_actor_label());check('twenty_seven_mechanisms',len(actors)==27)
  ...
  if phase==4:check(label+'_closed',d.get_open_fraction()<.001 and d.get_cam_release()==0 and not d.is_obstructed());s['index']+=1;advance(1);return
