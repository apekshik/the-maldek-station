"""Exercise all kitchen mechanisms with real E input, then walk staff routes."""
import unreal,json,time,traceback
from pathlib import Path
OUT=Path(__file__).resolve().parents[1];dest=OUT/'kitchen';shots=OUT/'previews/kitchen_night';shots.mkdir(exist_ok=True);ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);assert not ls.is_in_play_in_editor()
...
o=json.loads((OUT/'before.json').read_text())['origin'];data=json.loads((dest/'exports.json').read_text());specs=[('MIG_PLK_'+r['id']+'_'+p['role'],p) for r in data['groups'] for p in r['parts'] if p['control']]
s={'phase':0,'index':JOB.get('start',0),'next':time.monotonic()+12,'deadline':time.monotonic()+500,'checks':{},'images':[],'routes':[],'busy':False}
...
def put():
 ...
 f=d.focus_location;point=unreal.MathLibrary.transform_location(d.get_actor_transform(),unreal.Vector(f.x,-130 if ('Rear_right' in label or 'Fridge' in label) else -105,f.z));point.z=o[2]+499;p.set_actor_location(point,False,True)
def aim():
...
def tick(dt):
 ...
 try:
  ...
  if phase==3:key(False);advance(4,1.6);return
  if phase==4:
   s['last_motion']={'label':label,'fraction':d.get_open_fraction(),'obstructed':d.is_obstructed()}
   check(label+'_fully_open',abs(d.get_open_fraction()-1)<.001);check(label+'_not_obstructed',not d.is_obstructed());check(label+'_recorded_audio',bool(d.movement_sound) and bool(d.closing_sound));aim();shot(label+'_open');advance(5,1.2);return
  if phase==5:key(True);advance(6,.15);return
  if phase==6:key(False);advance(7,1.5);return
  if phase==7:check(label+'_closed_again',d.get_open_fraction()<.001);s['index']+=1;advance(1);return
  if phase==40:
   ...
   for a in unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationDoor):
    if a.get_actor_label().startswith('MIG_PLD_'):a.unlock();a.try_interact()
   advance(41,2);return
  if phase==41:
