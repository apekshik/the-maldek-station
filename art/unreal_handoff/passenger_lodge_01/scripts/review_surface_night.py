...
OUT=Path(__file__).resolve().parents[1];seating=globals().get('JOB',{}).get('seating',False);dest=OUT/('previews/seating_night' if seating else 'previews/surfaces_night');dest.mkdir(exist_ok=True);o=json.loads((OUT/'before.json').read_text())['origin']
...
if seating:views=[('hall',[-17.1,-6,5.65],[-17.1,2,4.7]),('table_detail',[-20.65,1.1,5.3],[-19.2,2.5,4.7]),('cross_aisle',[-22,1,5.65],[-12,1,4.7])]
...
def finish(error=None):
 ...
 (OUT/('seating/night_review.json' if seating else 'surface_night_review.json')).write_text(json.dumps({'error':error,'images':s['images'],'mode':'PIE; existing game night lighting and player flashlight; 2cm glancing camera shifts'},indent=2))
def tick(dt):
