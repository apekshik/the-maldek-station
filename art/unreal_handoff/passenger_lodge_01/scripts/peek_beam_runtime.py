...
def finish(error=None):
 ...
 s.update(success=error is None,error=error);(out/'peek_beam_runtime_final.json').write_text(json.dumps(s,indent=2));ls.editor_request_end_play()
...
def tick(dt):
 ...
 try:
  ...
  if n==0:
   ...
   check('torch_attached_to_closeup',beam.get_attach_parent()==cam)
   ...
   check('mouse_aims_beam',r.yaw>5 and r.pitch>3);check('beam_aim_limited',abs(r.yaw)<=24.1 and abs(r.pitch)<=18.1)
   ...
   r=beam.get_editor_property('relative_rotation');check('beam_returns_to_lock',abs(r.yaw)<.2 and abs(r.pitch)<.2)
