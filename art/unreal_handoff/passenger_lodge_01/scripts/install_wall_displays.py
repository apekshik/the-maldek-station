...
OUT=Path(__file__).resolve().parents[1];REPO=OUT.parents[2];dest=OUT/'wall_displays';root='/Game/MaldekRefinement/PassengerLodge/WallDisplays'
...
for row in data['groups']:
 ...
 for channel,trow in row['textures'].items():
  file=dest/trow['file'];assert hashlib.sha256(file.read_bytes()).hexdigest()==trow['sha256'];name='T_PLG_'+id+'_'+channel;path=root+'/Textures/'+name;tex=lib.load_asset(path)
  if not tex:
 ...
 name='M_PLG_'+id;path=root+'/Materials/'+name;m=lib.load_asset(path) or at.create_asset(name,root+'/Materials',unreal.Material,unreal.MaterialFactoryNew());ml.delete_all_material_expressions(m)
...
name='M_PLG_DisplayGlass';path=root+'/Materials/'+name;glass=lib.load_asset(path) or at.create_asset(name,root+'/Materials',unreal.Material,unreal.MaterialFactoryNew());ml.delete_all_material_expressions(glass)
...
name='M_PLG_Paper_edges';paper=lib.load_asset(root+'/Materials/'+name) or at.create_asset(name,root+'/Materials',unreal.Material,unreal.MaterialFactoryNew());ml.delete_all_material_expressions(paper);n=ml.create_material_expression(paper,unreal.MaterialExpressionConstant3Vector);n.set_editor_property('constant',unreal.LinearColor(.56,.49,.35,1));assert ml.connect_material_property(n,'',unreal.MaterialProperty.MP_BASE_COLOR);ml.recompile_material(paper);assert lib.save_loaded_asset(paper);printed['PLG_Paper_edges']=paper;printed['PLG_Case_glass']=glass
...
for row in data['groups']:
 for part in row['parts']:
  label='MIG_PLG_'+part['role'];M=part['matrix'];loc=unreal.Vector(o[0]-100*M[0][3],o[1]+100*M[1][3],o[2]+100*M[2][3]);rot=unreal.Rotator(yaw=180);a=actors.get(label) or aa.spawn_actor_from_class(unreal.StaticMeshActor,loc,rot);a.set_actor_label(label);a.set_folder_path('LodgeMigration/WallDisplays');a.static_mesh_component.set_static_mesh(meshes[part['mesh']]);a.static_mesh_component.set_collision_profile_name('BlockAll' if part['role']=='Static' else 'NoCollision');placed.append({'label':label,'mesh':part['mesh']})
