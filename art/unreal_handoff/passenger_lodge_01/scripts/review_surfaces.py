...
OUT=Path(__file__).resolve().parents[1];seating=globals().get('JOB',{}).get('seating',False);dest=OUT/('previews/seating' if seating else 'previews/surfaces');dest.mkdir(exist_ok=True);o=json.loads((OUT/'before.json').read_text())['origin']
...
def finish(error=None):
 ...
 (OUT/('seating/review.json' if seating else 'surface_review.json')).write_text(json.dumps({'views':rows,'error':error,'temporary_lights_removed':True,'mode':'Lit, temporary neutral review lights; live weather retained'},indent=2))
def tick(dt):
