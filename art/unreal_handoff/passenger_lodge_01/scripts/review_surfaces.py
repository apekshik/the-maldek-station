...
OUT=Path(__file__).resolve().parents[1];dest=OUT/'previews/surfaces';dest.mkdir(exist_ok=True);o=json.loads((OUT/'before.json').read_text())['origin']
...
def finish(error=None):
 ...
 (OUT/'surface_review.json').write_text(json.dumps({'views':rows,'error':error,'temporary_lights_removed':True,'mode':'Lit, temporary neutral review lights; live weather retained'},indent=2))
def tick(dt):
