...
b=Path(__file__).resolve().parents[1];out=b/'gondola_mechanism';views=out/'runtime_views';views.mkdir(exist_ok=True)
...
s={'phase':'warmup','next':time.monotonic()+15,'deadline':time.monotonic()+280,'busy':False,'samples':[],'max_relative_error_cm':0,'images':[],'max_rotor_vector_error':0,'max_rope_phase_error_m':0,'mechanism_samples':0}
...
def finish():
 ...
 (out/'runtime.json').write_text(json.dumps({k:z for k,z in s.items() if k not in ['g','parts','offsets','rotor_up']},indent=2));unreal.unregister_slate_post_tick_callback(handle)
def shot(name):
...
def tick(dt):
 ...
 try:
  ...
  manager=next(a for a in unreal.GameplayStatics.get_all_actors_of_class(w,unreal.GondolaMechanism))
  if s['phase']=='warmup':
   s['rotor_up']=[r.actor.get_actor_up_vector() for r in manager.rotors];assert len(s['rotor_up'])==84
   ...
   s['mechanism_samples']+=1
   s['max_rope_phase_error_m']=max(s['max_rope_phase_error_m'],abs(manager.rope_travel_meters-(distance-3048)/100))
   for r,up in zip(manager.rotors,s['rotor_up']):
    angle=math.degrees(manager.rope_travel_meters*100*r.ratio/r.pitch_radius_cm)%360
    expected=unreal.MathLibrary.rotate_angle_axis(up,angle,unreal.MathLibrary.normal(r.world_axis))
    error=(expected-r.actor.get_actor_up_vector()).length();s['max_rotor_vector_error']=max(s['max_rotor_vector_error'],error)
   assert s['max_rope_phase_error_m']<.03 and s['max_rotor_vector_error']<.005,(s['max_rope_phase_error_m'],s['max_rotor_vector_error'])
