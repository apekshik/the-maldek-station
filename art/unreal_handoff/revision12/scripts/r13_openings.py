...
b=Path(__file__).resolve().parents[1];out=b.parent/'revision13/reviews/openings';out.mkdir(parents=True,exist_ok=True)
...
def tick(dt):
 ...
 try:
  ...
  if s['phase']=='capture':
   p.set_actor_location(p.get_actor_location()+unreal.Vector(2,0,0),False,True)
