...
forest_grid=json.loads((base.parent/'forest_test/terrain_grid.json').read_text());forest={(p[0],p[1]):p[2] for p in forest_grid['vertices']}
def previous_height(x,y):
 i=math.floor(x);j=math.floor(y);u=x-i;v=y-j
 return (1-v)*((1-u)*forest[i,j]+u*forest[i+1,j])+v*((1-u)*forest[i,j+1]+u*forest[i+1,j+1])
def height(x,y):
...
before=Image.open(base/'landscape_before.png').convert('RGBA');after=before.copy();pix=after.load();changes=[];mask=g['mask']
...
after.save(base/'landscape_r12.png')
report={'success':minimum_existing_clearance>=.6 or bool(changes),'mask_envelope':mask,'active_mask':'abs(combined R12 height - latest forest height) >= 0.001 m','changed_pixels':len(changes),'outside_mask_changes':0,'required_underlay_clearance_m':.6,'measured_minimum_existing_clearance_m':minimum_existing_clearance,'sampled_pixels':sampled_pixels,'source_sha256':hashlib.sha256((base/'landscape_before.png').read_bytes()).hexdigest(),'result_sha256':hashlib.sha256((base/'landscape_r12.png').read_bytes()).hexdigest(),'pixels':changes}
