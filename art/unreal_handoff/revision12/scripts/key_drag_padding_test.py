...
out=Path(__file__).resolve().parents[1]/'pointer_key_test';out.mkdir(exist_ok=True);ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);assert not ls.is_in_play_in_editor()
...
def project(q):return pc.project_world_location_to_screen(d.get_key_grip_world_position(q))+unreal.Vector2D(0,48)
...
def tick(dt):
 ...
 try:
  ...
  phase=s['phase']
  if phase==0:
   unreal.StationMigrationLibrary.set_pie_render_size(1280,720);doors={a.get_actor_label():a for a in unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationDoor)};labels=sorted(n for n,a in doors.items() if a.has_key_lock);assert len(labels)>=1;labels=labels[:1]
  ...
  if phase==4:
   ...
   point(project(0)+unreal.Vector2D(-350,-250));key('LeftMouseButton',True);advance(5);return
