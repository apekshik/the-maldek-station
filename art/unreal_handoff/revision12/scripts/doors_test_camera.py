...
out=Path(__file__).resolve().parents[1]/'doors'/'camera';out.mkdir(exist_ok=True)
...
def tick(dt):
 ...
 try:
  ...
  if not p:return
  if s['phase']==0:
   unreal.StationMigrationLibrary.set_pie_render_size(1280,720);doors={a.get_actor_label():a for a in unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationDoor)};s['phase']=1
  if s['phase']==1:
  ...
  if s['phase']==5:
   check(d.get_actor_label()+'_reentry',d.is_using_keypad());file=out/(d.get_actor_label()+'.png');file.unlink(missing_ok=True);unreal.AutomationLibrary.take_high_res_screenshot(1280,720,str(file));s.update(phase=6,next=now+.5,clicks=[8,11,0,1,9,0,1,2,3,11],click_index=0);return
  if s['phase']==6:
  ...
  if s['phase']==9:
   check(d.get_actor_label()+'_door_opens_after_closeup',d.get_open_angle()>94);s['index']+=1;s.update(phase=1,next=now+.2)
