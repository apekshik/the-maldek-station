...
for key in ['viewport_size','console_settings','environment','cameras','pawn_settings']:assert before[key]==after[key],('Unpaired benchmark',key)
...
def summarize(rows):
 ...
 return result
report={'hardware':'RTX 5070 Ti, 16 GB','output_resolution':[2560,1440],'frame_generation':False,'scope':'Identical six fixed camera viewpoints and engine settings in initialized production Snow/night PIE. Engine counters measure real rendered frames.','settings':before['console_settings'],'cameras':{},'regressions':[]}
