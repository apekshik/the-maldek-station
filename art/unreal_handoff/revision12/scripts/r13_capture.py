...
base=Path(__file__).resolve().parents[1];out=base.parent/'revision13/reviews';out.mkdir(parents=True,exist_ok=True)
  ...
  if state['phase']=='switch_mode':
   ...
   for a in unreal.GameplayStatics.get_all_actors_of_class(w,unreal.Actor):
    ...
    for mc in a.get_components_by_class(unreal.StaticMeshComponent):
     if not mc.static_mesh or not mc.static_mesh.get_path_name().startswith(('/Game/MaldekRefinement/R12/Meshes/','/Game/MaldekRefinement/R13/Meshes/')):continue
