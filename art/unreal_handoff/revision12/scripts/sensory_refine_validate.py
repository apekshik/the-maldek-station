...
b=Path(__file__).resolve().parents[1];out=b/JOB.get('output','sensory_refine');out.mkdir(parents=True,exist_ok=True);ls=unreal.get_editor_subsystem(unreal.LevelEditorSubsystem);assert not ls.is_in_play_in_editor()
...
def finish():
 ...
 if JOB.get('check_held'):s['checks']['torch_stable_relative_to_view']=bool(s.get('held_error_cm')) and max(s['held_error_cm'])<.2
...
def tick(dt):
 ...
 try:
  ...
  if JOB.get('check_held') and s['phase'] in ['idle','walk','run']:
   held=next(v for v in p.get_components_by_class(unreal.SceneComponent) if v.get_name()=='HeldTorchMotion')
   s.setdefault('held_error_cm',[]).append((held.get_editor_property('relative_location')-c.get_view_offset()).length())
  if s['phase']=='start':
  ...
  if s['phase']=='idle':
   ...
   s['checks']['idle_sway_bounded']=max(math.hypot(r[1],r[2]) for r in s['idle_samples'])<JOB.get('idle_max',1.1)
   s['checks']['idle_breathing_present']=max(r[2] for r in s['idle_samples'])-min(r[2] for r in s['idle_samples'])>JOB.get('idle_min_range',1.3)
   ...
   if JOB.get('capture_torch'):unreal.AutomationLibrary.take_high_res_screenshot(1400,800,str(out/'in_game.png'))
   unreal.AudioMixerLibrary.start_recording_output(w,25);s.update(phase='walk',started=now,next=now,direction=1);return
  if s['phase'] in ['walk','run']:
   ...
   if now-s['started']>(9 if phase=='walk' else 12):
    if phase=='walk':
    ...
    unreal.AudioMixerLibrary.stop_recording_output(w,unreal.AudioRecordingExportType.WAV_FILE,'walking_mix',str(out));unreal.StationMigrationLibrary.send_pie_key('LeftShift',False);m.stop_movement_immediately();s.update(phase='stop',next=now+2);return
   return
  if s['phase']=='stop':
   s['checks']['release_walk_speed']=abs(m.max_walk_speed-350)<1;s['checks']['camera_returns_to_idle_after_run']=c.get_view_offset().length()<JOB.get('idle_max',1.1)
   for name,rows in s['samples'].items():
    s['checks'][name+'_grounded']=all(not r[3] for r in rows);s['checks'][name+'_bob_bounded']=max(abs(r[2]) for r in rows)<3
   def hz(rows):
