...
trims=[o for o in bpy.data.objects if o.type=='MESH' and o.name.startswith(('Door_return','Door_lintel'))]
edits=[];planes={}
for t in trims:
 lo,hi=bounds(t);axis=2 if t.name.startswith('Door_lintel') else min(range(2),key=lambda k:hi[k]-lo[k])
 for c in cores:
  a,z=bounds(c)
...
for i,row in enumerate(list(manifest['chunks'])):
 if not set(row['sources'])&set(planes):continue
...
(OUT/'handoff_manifest.json').write_text(json.dumps(manifest,indent=2));h.save('door_core_recess_export')
(OUT/'trim2/core_recess.json').write_text(json.dumps({'source_sha256':EXPECTED,'approved_source_modified':False,'edits':edits,'changed_assets':changed,'collision_unchanged':True},indent=2));print('RECESSED',len(planes),'CORES;',len(changed),'ASSEMBLIES',flush=True)
