...
w=unreal.get_editor_subsystem(unreal.UnrealEditorSubsystem).get_editor_world();a.set_actor_location(start+unreal.Vector(210,-425,0),False,True)
a.set_actor_rotation(unreal.Rotator(yaw=180),True)
heights=[]
...
sight=[]
for eye in [start+unreal.Vector(0,-740,170),start+unreal.Vector(0,-550,170)]:
...
RESULT={'success':True,'position':a.get_actor_location().to_tuple(),'support_heights':heights,'approach_sightlines_clear':sight,'placement':'Beside the boarding-edge railing, left of the gondola and outside the central approach'};(out/'placement.json').write_text(json.dumps(RESULT,indent=2))
