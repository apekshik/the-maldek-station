...
rows=[]
for o in bpy.data.collections['VF06_Relay_Shell'].objects:
 if o.name.startswith(('Door_return','Door_lintel','Flush_threshold')):
  ...
  rows.append({'name':o.name,'min':[min(v[i] for v in p) for i in range(3)],'max':[max(v[i] for v in p) for i in range(3)]})
(root/'art/unreal_handoff/revision12/doors/relay_openings.json').write_text(json.dumps(rows,indent=2))
