...
out=Path(__file__).resolve().parents[1]/'doors'/'keypad_previews';out.mkdir(exist_ok=True)
...
def tick(dt):
 ...
 try:
  ...
  if s['phase']==0:
   ...
   doors=list(unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationDoor));front=next(a for a in doors if a.get_actor_label()=='R12_Door_Control_front');side=next(a for a in doors if a.get_actor_label()=='R12_Door_Relay_south');north=next(a for a in doors if a.get_actor_label()=='R12_Door_Relay_north')
   for d,n in [(front,'control'),(side,'relay_south'),(north,'relay_north')]:
  ...
  if s['phase']==1:
   ...
   lamp.set_actor_hidden_in_game(mode!='neutral');lp=unreal.MathLibrary.transform_location(t,unreal.Vector(-30,180,240));lamp.set_actor_location(lp,False,True);lamp.set_actor_rotation(unreal.MathLibrary.find_look_at_rotation(lp,target),False)
