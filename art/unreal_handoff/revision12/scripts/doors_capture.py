...
out=Path(__file__).resolve().parents[1]/'doors'/'previews';out.mkdir(exist_ok=True)
...
def tick(dt):
 ...
 try:
  ...
  if s['phase']==0:
   ...
   doors=list(unreal.GameplayStatics.get_all_actors_of_class(w,unreal.StationDoor));front=next(a for a in doors if abs(a.get_actor_rotation().yaw)<1);side=next(a for a in doors if a!=front)
   for d,n in [(front,'front'),(side,'side')]:
