...
OUT=Path(__file__).resolve().parents[1]/'renders';OUT.mkdir(exist_ok=True)
...
volume.set_editor_property('unbound',True);volume.set_editor_property('priority',100)
settings=volume.get_editor_property('settings')
settings.set_editor_property('override_auto_exposure_bias',True);settings.set_editor_property('auto_exposure_bias',2.0)
...
def tick(delta):
 try:
  ...
  i=state['index']
  if i>=len(shots):
   (OUT/'capture_report.json').write_text(json.dumps({'engine':'Unreal Engine 5.7','map':'BlockOut_R04','resolution':[2560,1440],'preview_exposure_bias_stops':2,'map_saved':False,'shots':state['shots']},indent=2))
