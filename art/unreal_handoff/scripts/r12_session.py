...
out=Path(__file__).resolve().parents[1]/'revision12'
previous=getattr(builtins,'r12_dispatch_handle',None)
if previous:
...
def tick(dt):
 ...
 try:
  ...
  script=(out/'scripts'/job['script']).resolve()
  assert script.parent==(out/'scripts').resolve() and script.suffix=='.py'
...
builtins.r12_dispatch_handle=handle
