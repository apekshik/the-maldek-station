...
OUT=Path(__file__).resolve().parents[1]/'revision06'/'final_renders';OUT.mkdir(exist_ok=True)
...
shots=[
 ...
 ('05_lower_drive',[-45400,20400,10400],[-44300,19000,9750]),
 ('09_cliff_drop',[-43750,20300,10650],[-44200,19400,8850]),
...
def tick(delta):
 try:
  ...
  i=state['index']
  if i>=len(shots):
   (OUT/'capture_report.json').write_text(json.dumps({'engine':'Unreal Engine 5.7','map':'BlockOut_R06','resolution':[2560,1440],'preview_exposure_bias_stops':0,'map_saved':False,'shots':state['shots']},indent=2))
