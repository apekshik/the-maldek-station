...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'revision06'
s=json.loads((ROOT/'revision05/terrain_samples.json').read_text());h=s['heights']
...
for y in range(ymin,ymax+1):
 for x in range(xmin,xmax+1):
  ...
  gorge=blend(max(0,abs(x-1)-7),10)*(1-blend(y-2,10))*blend(max(0,y-65),28)
  z=z+(min(z,-24+.5*math.sin(y*.12))-z)*gorge
  verts.append((x,y,z))
for j in range(ny-1):
 for i in range(nx-1):
...
o=bpy.data.objects.new('SM_R06_Local_Terrain',me);bpy.context.collection.objects.link(o)
...
bpy.ops.export_scene.fbx(filepath=str(OUT/'fbx/SM_R06_Local_Terrain.fbx'),use_selection=True,object_types={'MESH'},axis_forward='-Y',axis_up='Z',apply_unit_scale=True,apply_scale_options='FBX_SCALE_UNITS',bake_anim=False,mesh_smooth_type='FACE')
(OUT/'terrain_grid.json').write_text(json.dumps({'xmin':xmin,'xmax':xmax,'ymin':ymin,'ymax':ymax,'nx':nx,'vertices':verts}))
