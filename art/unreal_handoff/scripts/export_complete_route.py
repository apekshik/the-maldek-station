...
OUT=Path(__file__).resolve().parents[1]/'revision05';OUT.mkdir(exist_ok=True)
FBX=OUT/'fbx'; FBX.mkdir(exist_ok=True)
source=bpy.context.scene
deps=bpy.context.evaluated_depsgraph_get()
manifest={'source':bpy.data.filepath,'units':'meters','materials':{},'chunks':[]}
collections=[c for c in bpy.data.collections if c.name[:2] in ['07','09','13']]
groups={}
for col in collections:
 for o in col.all_objects:
  if o.type not in {'MESH','FONT'}:continue
  if 'Temporary_Cut' in o.name:continue
  if o.name.startswith('Continuous_Relay_Path_'):o['export_geometry']=True;o['collision']=False
  key=col.name
