...
OUT=Path(__file__).resolve().parents[1];bpy.ops.wm.open_mainfile(filepath=str(OUT/'Maldek_Generator_Fuel_Refinement.blend'))
dg=bpy.context.evaluated_depsgraph_get();verts=[];faces=[];objects=[]
for c in bpy.data.collections:
 if not c.name.startswith('VF08_') or c.name.endswith('Presentation'):continue
 for o in c.objects:
...
for name,points in routes.items():
 ...
 for a,b in zip(points,points[1:]):
  ...
  for j in range(n+1):
   p=Vector((a[0]+dx*j/n,a[1]+dy*j/n,-1));count+=1
   for side in [-.28,0,.28]:
    q=p+perp*side;hit=bvh.ray_cast(q+Vector((0,0,.3)),Vector((0,0,-1)),.65)
    if hit[0] is None or abs(hit[0].z+1)>.06:failures.append({'p':list(q),'reason':'floor'})
...
report=json.loads((OUT/'build_report.json').read_text());anchors=[]
...
r={'evaluated_objects':len(objects),'vertices':len(verts),'polygons':len(faces),'routes':results,'anchors':anchors,'scope':'Blender sampled visible floor/body clearances; not Unreal capsule verification','passed':all(x['passed'] for x in results+anchors)}
