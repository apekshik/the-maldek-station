...
for c in source.collection.children:
    if not c.name[:2].isdigit() or int(c.name[:2])>=90 or c.name[:2] in ['15','16','17']: continue
    ...
    if c.name=='11_Doors':
        ...
        groups['SM_Millford_Gondola']={'objects':objects,'pivot':Vector((0,8.05,4)),'kind':'movable gondola'}
