...
checks['outside_boarding_aisle']=min(p.x for p in corners)>1.7
...
report={'success':all(checks.values()),'checks':checks,'mesh_objects':count,'nonmanifold':bad,'circuit_strengths':strengths,'minimum_sign_world_x_m':min(p.x for p in corners),'bounds_world_m':[[min(p[i] for p in corners),max(p[i] for p in corners)] for i in range(3)],'integration':'Not imported; game validation deferred until design approval'}
