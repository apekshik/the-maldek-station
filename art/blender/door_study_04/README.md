...
Open `Maldek_Keyed_Door.blend` and play frames 1–120 at 24 fps: key ready, insertion, 90-degree turn, return, withdrawal, then door opening. The cylinder, key and latch have separate animation controls. No hand animation is included. The old cylinder/painted slot is removed; both opaque door layers and both handle plates have a real cylinder bore. The Unreal integration uses separate exports and preserves the installed walking collision; see ../../unreal_handoff/revision12/doors/key_lock/README.md.

Initial gameplay intent: make the matching key available automatically. Later, key possession can gate this same animation without redesigning the mechanism. The game integration exposes a required-key identifier and grants access by default; finding keys is not required yet.
