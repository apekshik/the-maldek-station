...
Stair railing correction: run scripts/repair_stair_rails.py after finish_edges.py and before verify.py. It removes source guard fragments crossing the flight, installs a continuous 1.1 m left deck guard, and rebuilds both pitched handrails with landing extensions. Three close-up cameras are saved; stair_rail_repair.json records removed source links and the crossing check. Source guard objects remain intact in reference scenes.

Upper landing circulation: guards stop at the stair opening (y = -7.7 m). No enclosure crosses the level route toward the right platform. Verification checks that route with horizontal rays at three offsets and three body heights, plus three floor/headroom samples on every stair tread. Stair_Right_Access is a perspective view from the right platform. These are Blender geometry checks, not Unreal capsule tests.
