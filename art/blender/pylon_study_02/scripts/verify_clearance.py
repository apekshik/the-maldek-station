...
bpy.ops.wm.open_mainfile(filepath=str(out/'Maldek_Weathered_Central_Pylon.blend'),load_ui=False)
...
for dy in [t+7 for t in travel]:
 for o in cabin:
  vs,fs=meshdata(o,dy)
  if not fs:continue
  ct=BVHTree.FromPolygons(vs,fs,all_triangles=True)
  overlap=pt.overlap(ct)
  if overlap:collisions.append({'cabin':o.name,'travel_y':dy-7,'pylon_parts':sorted(set(owners[a] for a,b in overlap)),'triangle_pairs':len(overlap)})
report={'passenger_lanes':1,'main_sheaves':12,'bare_return_rollers':4,'tested_cabin_travel_positions_m':travel,'collisions':collisions,'tower_bounds_m':[[min(p[i] for p in pv) for i in range(3)],[max(p[i] for p in pv) for i in range(3)]],'pylon_triangles':len(pf),'cabin_reference_objects':len(cabin),'production_gondola_modified':False,'runtime_validation':'Pending Unreal terrain, spline and rig integration'}
