...
OUT=Path(__file__).resolve().parents[1];f=OUT/'Maldek_Lodge_Shell_Polish.blend'
bpy.ops.wm.open_mainfile(filepath=str(f));s=bpy.data.scenes['Shell_Polish_Review'];bpy.context.window.scene=s;bpy.context.view_layer.update();dg=bpy.context.evaluated_depsgraph_get()
r=json.loads((OUT/'shell_manifest.json').read_text());assert r['source_unchanged']
for name,bb in r['original_bounds'].items():
 o=bpy.data.objects[name];ps=[o.matrix_world@Vector(v) for v in o.bound_box]
 actual=[[min(v[i] for v in ps) for i in range(3)],[max(v[i] for v in ps) for i in range(3)]]
 assert all(abs(actual[j][i]-bb[j][i])<1e-5 for i in range(3) for j in range(2)),name
for im in bpy.data.images:
 if im.name.startswith('plastered_wall_04'):
  assert Path(bpy.path.abspath(im.filepath)).is_file(),im.filepath
  assert im.size[0]>=4096,im.name
bad=[];count=0
for cn in ['PL03_Open_Grating','PL03_Corrugated_Exterior','PL03_Furniture_Details','PL03_Removable_Roof','PLSH_Shell_Trim']:
